...
(defn sat-solve-find-all-results-dimacs
  ...
  (set (loop
         ...
         (let [res (sat-solve-dimacs d)]
           (if (= 0 (first res))
        results
        (recur (add-negated-dimacs-result d res) (conj results (subvec res 1))))))))

(defn sat-solve-find-all-results
  "Returns all result as sequence of vector with litereals."
  [dimacs-map]
  (set (map (partial replace-number-with-literal dimacs-map) (sat-solve-find-all-results-dimacs dimacs-map))))
