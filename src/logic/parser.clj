...
(defn init-parser
  "Safley either constructs or reinits the MpaParser."
  [stream]
  (try
    (MpaParser/ReInit stream)
    (catch NullPointerException _ (MpaParser. stream))))
...
(defn javaCCparse
  [formula]
  (init-parser (StringReader. formula))
...
(defn parse
  ...
  [formula]
  (remove-constants (javaCCparse formula)))